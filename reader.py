...
def replaceUnknown(filename):
    ...
    file = open(filename, 'r', encoding='utf-8')
    fileOutput = open("/Users/apple/Desktop/NLP/textclassfier/text/data_fenci/finalData/replaceUnknown.txt", "w", encoding='utf-8')
    lines = list(file.readlines())
    for i in range(len(lines)):
        line = lines[i].split(' ')
...
# min_count: specifies the minimum amount of times that the word has to appear in the corpus before it is included in the vocabulary
# window: The maximum distance between the current and predicted word within a sentence.
# size: the length of the word vector
